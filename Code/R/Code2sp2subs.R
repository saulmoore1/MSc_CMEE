...
for (i in (1:length(times))) {
	...
	S[1] <- S[1]-(kcat[1,1]*E[1,1]*S[1]*pH/(Km[1,1]+S[1]))*N[1]*time.step-(kcat[1,2]*E[1,2]*S[1]*pH/(Km[1,2]+S[1]))*N[2]*time.step-d*S[1]*time.step+d*S0[1]*time.step
	S[2] <- S[2]-(kcat[2,1]*E[2,1]*S[2]*pH/(Km[2,1]+S[2]))*N[1]*time.step-(kcat[2,2]*E[2,2]*S[2]*pH/(Km[2,2]+S[2]))*N[2]*time.step-d*S[2]*time.step+d*S0[2]*time.step
	N[1] <- N[1]+(c.conv[1,1]*(kcat[1,1]*E[1,1]*S[1]*pH/(Km[1,1]+S[1]))*N[1]*time.step)+(c.conv[2,1]*(kcat[2,1]*E[2,1]*S[2]*pH/(Km[2,1]+S[2]))*N[1]*time.step)-d*N[1]*time.step
	N[2] <- N[2]+(c.conv[1,2]*(kcat[1,2]*E[1,2]*S[1]*pH/(Km[1,2]+S[1]))*N[2]*time.step)+(c.conv[2,2]*(kcat[2,2]*E[2,2]*S[2]*pH/(Km[2,2]+S[2]))*N[2]*time.step)-d*N[2]*time.step
...
matplot(results[,1], results[,2:5], type="l", xlab="Time", 
        ...
        lty=c(1,1,2,2))
legend("topright",legend=c("S1","S2","N1","N2"), col=c("black","red","black","red"),lty=c(1,1,2,2))
